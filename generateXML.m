...
save(cropInfo, 'xObj', 'yObj', 'xMask', 'yMask', 'scaleSphere');
...
envName = strrep(envName, '.npz', '.mat');
env = load(envName ); env = env.env;
env = flip(env, 3);
hdrwrite(max(env, 0), strrep(envName, 'mat', 'hdr') );
...
mask1 = mask1 == 1;
mask1 = imerode(mask1, strel('disk', 2)); 
